...
from forms import CallEventForm, EventForm, MeetingEventForm, TaskEventForm
...
def load_meetings(request):
    if request.method == "GET":
        ...
        meetings = Event.objects.filter(
            Q(created_user=request.user) | Q(updated_user=request.user),
            event_type="Meeting",
        ).order_by("-id")
...
def load_calls(request):
    if request.method == "GET":
        ...
        try:
            ...
            return render(request, "calls_list.html", {"calls": calls})
        except PageNotAnInteger:
        ...
        return render(
            request,
            "calls.html",
            {"calls": calls, "reminder_form_set": reminder_form_set},
        )
    else:
...
def create_meeting(request):
    if request.method == "POST":
        ...
        if eventForm.is_valid() and reminder_form_set.is_valid():
            parent_name = None
            if eventForm.cleaned_data.get("parent_type") and eventForm.cleaned_data.get(
                "parent_id"
            ):
                meeting_object = add_parent(
                    eventForm.cleaned_data.get("parent_type"),
                    eventForm.cleaned_data.get("parent_id"),
                    eventForm,
                    request.user,
                )
...
def create_task(request):
    if request.method == "POST":
        ...
        if taskForm.is_valid():
            if taskForm.cleaned_data.get("parent_type") and taskForm.cleaned_data.get(
                "parent_id"
            ):
                task_object = add_parent(
                    taskForm.cleaned_data.get("parent_type"),
                    taskForm.cleaned_data.get("parent_id"),
                    taskForm,
                    request.user,
                )
                task = task_object[0]
...
def create_call(request):
    if request.method == "POST":
        ...
        if callsForm.is_valid() and reminder_form_set.is_valid():
            parent_name = None
            if callsForm.cleaned_data.get("parent_type") and callsForm.cleaned_data.get(
                "parent_id"
            ):
                call_object = add_parent(
                    callsForm.cleaned_data.get("parent_type"),
                    callsForm.cleaned_data.get("parent_id"),
                    callsForm,
                    request.user,
                )
                call = call_object[0]
...
def delete_meeting(request):
    if request.method == "POST":
        try:
            Event.objects.get(
                id=int(request.POST.get("meetingID")), created_user=request.user
            ).delete()
...
def delete_task(request):
    if request.method == "POST":
        try:
            Event.objects.get(
                id=int(request.POST.get("taskID")), created_user=request.user
            ).delete()
...
def delete_call(request):
    if request.method == "POST":
        try:
            Event.objects.get(
                id=int(request.POST.get("callID")), created_user=request.user
            ).delete()
...
def get_meeting(request):
    if request.method == "POST":
        try:
            meeting = Event.objects.get(
                id=int(request.POST.get("meetingID")), created_user=request.user
            )
            ReminderFormSet = modelformset_factory(
                Reminder, ReminderForm, can_delete=True, extra=0
            )
            reminder_form_set = ReminderFormSet(queryset=meeting.reminders.all())
            remindersHtML = render(
                request, "reminder.html", {"reminder_form_set": reminder_form_set}
            )
            data = {}
            if meeting.parent:
                ...
                data["parent_name"] = ""
                data["parent_id"] = ""
            if meeting.assigned_users:
                data["assigned_users"] = {
                    meeting.assigned_users.id: meeting.assigned_users.username
                }
            if meeting.attendees_user:
                data["attendees_user"] = {
                    key.id: key.username for key in meeting.attendees_user.all()
                }
            if meeting.attendees_contacts:
                data["attendees_contacts"] = {
                    key.id: key.name for key in meeting.attendees_contacts.all()
                }
            if meeting.attendees_leads:
                data["attendees_leads"] = {
                    key.id: key.first_name for key in meeting.attendees_leads.all()
                }
            del meeting._state
            del meeting._parent_cache
            del meeting._assigned_users_cache
            meeting.__dict__["close_date"] = meeting.__dict__["close_date"].strftime(
                "%m/%d/%Y %H:%M:%S"
            )
            meeting.__dict__["start_date"] = meeting.__dict__["start_date"].strftime(
                "%m/%d/%Y %H:%M:%S"
            )
...
def get_task(request):
    if request.method == "POST":
        try:
            ...
            task.__dict__["close_date"] = task.__dict__["close_date"].strftime(
                "%m/%d/%Y %H:%M:%S"
            )
            task.__dict__["start_date"] = task.__dict__["start_date"].strftime(
                "%m/%d/%Y %H:%M:%S"
            )
    ...
    if request.method == "POST":
        try:
            ...
            if call.parent:
                ...
                data["parent_name"] = ""
                data["parent_id"] = ""
            if call.assigned_users:
                data["assigned_users"] = {
                    call.assigned_users.id: call.assigned_users.username
                }
            if call.attendees_user:
                data["attendees_user"] = {
                    key.id: key.username for key in call.attendees_user.all()
                }
            if call.attendees_contacts:
                data["attendees_contacts"] = {
                    key.id: key.name for key in call.attendees_contacts.all()
                }
            if call.attendees_leads:
                data["attendees_leads"] = {
                    key.id: key.first_name for key in call.attendees_leads.all()
                }
            del call._state
            del call._parent_cache
            del call._assigned_users_cache
            call.__dict__["start_date"] = call.__dict__["start_date"].strftime(
                "%m/%d/%Y %H:%M:%S"
            )
...
def update_meeting(request):
    if request.method == "POST":
        try:
            ...
            ReminderFormSet = modelformset_factory(
                Reminder, ReminderForm, can_delete=True
            )
            reminder_form_set = ReminderFormSet(request.POST)
            if eventForm.is_valid() and reminder_form_set.is_valid():
                meeting = None
                parent_name = None
                if eventForm.cleaned_data.get(
                    "parent_type"
                ) and eventForm.cleaned_data.get("parent_id"):
                    meeting_object = add_parent(
                        eventForm.cleaned_data.get("parent_type"),
                        eventForm.cleaned_data.get("parent_id"),
                        eventForm,
                ...
                if reminder_form_set.errors:
                    return JsonResponse(
                        {"reminder": reminder_form_set.errors[-1]["__all__"][0]}
                    )
...
def update_call(request):
    if request.method == "POST":
        try:
            ...
            if callsForm.is_valid() and reminder_form_set.is_valid():
                parent_name = None
                if callsForm.cleaned_data.get(
                    "parent_type"
                ) and callsForm.cleaned_data.get("parent_id"):
                    call_object = add_parent(
                        callsForm.cleaned_data.get("parent_type"),
                        callsForm.cleaned_data.get("parent_id"),
                        callsForm,
    ...
    if request.method == "GET":
        try:
            ...
            if request.GET.get("parent_type") and request.GET.get("parent_id"):
                parent = get_parent(
                    request.GET.get("parent_type"), request.GET.get("parent_id")
                )
                if parent is not None:
                    parent_content_type = ContentType.objects.get_for_model(parent)
                    event = (
                        event.filter(
                            content_type__pk=parent_content_type.id, object_id=parent.id
                        )
                        or event
                    )
            ...
            if request.GET.getlist("attendees_contacts"):
                event = (
                    event.filter(
                        attendees_contacts__in=request.GET.getlist("attendees_contacts")
                    )
                    or event
                )
            if request.GET.getlist("attendees_user"):
                event = (
                    event.filter(
                        attendees_user__in=request.GET.getlist("attendees_user")
                    )
                    or event
                )
            if request.GET.getlist("attendees_leads"):
                event = (
                    event.filter(
                        attendees_leads__in=request.GET.getlist("attendees_leads")
                    )
                    or event
                )
            ...
            if request.GET.get("assigned_users"):
                event = (
                    event.filter(assigned_users=request.GET.get("assigned_users"))
                    or event
                )
            if request.GET.get("created_user"):
                event = (
                    event.filter(created_user=request.GET.get("created_user")) or event
                )
            if request.GET.get("updated_user"):
                event = (
                    event.filter(updated_user=request.GET.get("updated_user")) or event
                )
            ...
            if request.GET.get("event_type") == "Meeting":
                try:
                    ...
                    return render(request, "meetings_list.html", {"meetings": event})
                except PageNotAnInteger:
                ...
                try:
                    event = paginator.page(request.GET.get("page"))
                    return render(request, "calls_list.html", {"calls": event})
                except PageNotAnInteger:
                    event = paginator.page(1)
                except EmptyPage:
                    event = paginator.page(paginator.num_pages)
                ...
                try:
                    event = paginator.page(request.GET.get("page"))
                    return render(request, "tasks_list.html", {"tasks": event})
                except PageNotAnInteger:
                    event = paginator.page(1)
                except EmptyPage:
                    event = paginator.page(paginator.num_pages)
